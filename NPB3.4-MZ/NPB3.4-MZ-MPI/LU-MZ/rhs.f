...
c---------------------------------------------------------------------
      call exchange_3(u, nx, nxmax, ny, nz)
...
!$OMP DO SCHEDULE(runtime)
      do k = 2, nz - 1
         do i = ist, iend
            do j = 1, ny
               ...
               utmp(4,j) = u(4,i,j,k)
               utmp(5,j) = u(5,i,j,k)
               utmp(6,j) = rho_i(i,j,k)
            end do

            do j = 1, ny
               flux(1,j) = utmp(3,j)
               u31 = utmp(3,j) * utmp(6,j)

               q = qs(i,j,k)

               flux(2,j) = utmp(2,j) * u31 
               flux(3,j) = utmp(3,j) * u31 + c2 * (utmp(5,j)-q)
               flux(4,j) = utmp(4,j) * u31
               flux(5,j) = ( c1 * utmp(5,j) - c2 * q ) * u31
            end do
     ...
     >                   - ty2 * ( flux(m,j+1) - flux(m,j-1) )
            ...
            do j = 2, ny
               ...
               tmp = utmp(6,j-1)
               u21jm1 = tmp * utmp(2,j-1)
               u31jm1 = tmp * utmp(3,j-1)
               u41jm1 = tmp * utmp(4,j-1)
               u51jm1 = tmp * utmp(5,j-1)
