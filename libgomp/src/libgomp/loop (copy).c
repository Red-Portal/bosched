		...
		enum gomp_schedule_type sched, long chunk_size, unsigned num_threads)
{
  ...
  case GFS_WFAC:
   ...
   //printf("hi");
	...
	ws->WFACworkload = 0;
	//printf("numthreads: %i, thrnum: %i\n, wfacinit: %i\n\n", num_threads,omp_get_thread_num(), WFACinit);	
	//printf("wfaci: %i\n\n", WFACinit);
	//if (WFACinit > 0){ printf("init done");} 
  ...
  double boldsigma = 0;
  boldsigma = strtod(getenv("BOLDSIGMA"),NULL);
  ws->boldmue = strtod(getenv("BOLDMUE"),NULL);
   ...
   case GFS_TRAP:
   {
	...
	ws->startsize = 0;	
	 // printf("Trapezoid started \n");
	  ...
	  ws->startsize = atoi(getenv("TRAPSTART"));
...
//	  printf("start: %i, end: %i, numthr: %i\n",startsize,endsize, num_threads);
  ...
  case GFS_FSC:
  {
	...
	double sigma = strtod(getenv("FSCSIGMA"),NULL);
