...
gomp_iter_dynamic_next (long *pstart, long *pend)
{
  ...
  if (__builtin_expect (ws->mode, 1))
      ...
      if (incr > 0)
	{
     ...
     // if(mycounter == ws->maxiters){
    //     double diff1 = diff( ws->dyntime,mytime);
      //   ws->dynarray[mycounter] = diff1;
//double mean = 0;
//double stddev = 0;
//for(long i = 1; i < mycounter; i++){
 // mean = mean + fabs(ws->dynarray[i]);
  //printf("%ld time0 taken: %f, max: %lld\n",i, ws->dynarray[i], ws->maxiters); 
//}
//if(mycounter > 1) mean = mean/(mycounter-1);
//for(long i = 1; i < mycounter; i++){
//stddev = stddev + ( ( fabs(ws->dynarray[i])-mean )* ( fabs(ws->dynarray[i])-mean ) ) ;
//}
//if(mycounter > 1) stddev = stddev/(mycounter-1);
//stddev = sqrt(stddev);
//printf("mean: %f, stddev: %f,counter: %d\n",mean,stddev,mycounter);
...
bool
gomp_iter_awf_next(long *pstart, long *pend)
{

return true;}
bool
gomp_iter_fac2_next(long *pstart, long *pend)
{
...
bool
gomp_iter_rand_next(long *pstart, long *pend)
{
  struct gomp_thread *thr = gomp_thread ();
  struct gomp_work_share *ws = thr->ts.work_share;
  ...
  while (1)
	...
	return false;
	  ...
	  q = (rand() % (ws->randmax-ws->randmin)) + ws->randmin;
      if (q < chunk_size)
	q = chunk_size;
      if (__builtin_expect (q <= n, 1))
	nend = start + q;
...
//printf("cpu: %d, thrn: %d\n",sched_getcpu(), omp_get_thread_num());
//printf("%ld rand ,q: %ld, n: %ld, thrNr: %i, end: %ld,  nend: %ld\n", start, q, n, omp_get_thread_num(), end,  nend);
break;}
...
gomp_iter_fact_next (long *pstart, long *pend)
{
  ...
  while (1)
    ...
    Rj = ws->maxworkload;
    bj = (nthreads/(2*sqrt(Rj)))*(ws->facsigma/ws->facmue);
     
    xj = 1+bj*bj+bj*sqrt(bj*bj+2);
    Fj = Rj/(xj*nthreads);
 
	for (int i = 1; i < blatest; i++){
	    Rj -= Fj;
	    bj = (nthreads/(2*sqrt(Rj)))*(ws->facsigma/ws->facmue);
	    xj = 2+bj*bj+bj*sqrt(bj*bj+4);
	    Fj = Rj/(xj*nthreads);
	    }
...
//printf("cpu: %d, thrn: %d\n",sched_getcpu(), omp_get_thread_num());
//printf("%ld fact ,q: %ld, n: %ld, thrNr: %i, end: %ld,  nend: %ld, blatest: %i\n", start, q, n, omp_get_thread_num(), end,  nend, blatest);
break;}
...
//chunk_size=1;
  ...
  while (1)
	...
	double q;
      long tmp;
	double Ti = (double)(end - start)/(double)nthreads;
      if (start == end)
	return false;
      n = (end - start); /// incr;
	  //if (globalfactcounter%nthreads == 0) {
	  q =  (Ti + ((ws->va*ws->va)/2.0) - (ws->va*sqrt(2.0*Ti+((ws->va*ws->va)/4.0))));
	  if (q<=1) q=1;
...
//printf("q2:%f\n",q);
      ...
      if (__builtin_expect (tmp == start, 1)){
//printf("%ld,",q);
//printf("%ld tape ,q: %f, n: %ld, thrNr: %i, end: %ld,  nend: %ld, va: %f\n", start, q, n, omp_get_thread_num(), end,  nend, ws->va);
break;}
	  //printf("\nhi\n");
...
gomp_iter_wfac_next (long *pstart, long *pend)
{
 ...
 while (1)
	...
	nend = end;
      ...
      if (__builtin_expect (tmp == start, 1)){
//printf("%ld,",q);
//printf("%ld wfac ,q: %ld, n: %ld, thrNr: %i, end: %ld, incr: %ld, nend: %ld, multi: %f\n", start, q, n, omp_get_thread_num(), end, incr, nend,ws->WFACarray[omp_get_thread_num()]);
break;}
	  //printf("\nhi\n");
	 // __sync_sub_and_fetch(&globalfactcounter, 1);
      start = tmp;
...
//pthread_mutex_lock(&lockm);
	__sync_sub_and_fetch(&ws->boldm, (long long) ws->boldarray[omp_get_thread_num()]);
//pthread_mutex_unlock(&lockm);
//pthread_mutex_lock(&lockspeed);
	__sync_sub_and_fetch(&ws->totalspeed, (long long)ws->speedarray[omp_get_thread_num()]);
//pthread_mutex_unlock(&lockspeed);
  ...
  while (1)
	...
	if(q<=1){
	  ...
	  double d = n/(1.0+(1.0/ln_Q)-v);
	  if(d<=ws->c2){ 
		q = t;
	  }
	  else{	
		double s = ws->bolda*(log(d)-ws->c3)*(1.0+(ws->boldm/(double)(r*nthreads)));
	  	double w = 0;
...
//pthread_mutex_lock(&lockn);
	__sync_sub_and_fetch(&ws->boldn, (long long) (diff1*ws->totalspeed+q-((diff2*q)/(q*ws->boldmue+ws->boldh))));
...
//printf("%ld,",q);
//printf("%ld bold , chunk_size: %ld, q: %f, n: %ld, thrNr: %i, end: %ld,  nend: %ld, totalspeed: %lld, boldn: %lld, speedarray: %f, diff1: %f, diff2: %f\n", start, chunk_size, q, n, omp_get_thread_num(), end,  nend, ws->totalspeed, ws->boldn, ws->speedarray[omp_get_thread_num()], diff1, diff2);
break;}
	  //printf("\nhi\n");
	 // __sync_sub_and_fetch(&globalfactcounter, 1);
      start = tmp;
...
gomp_iter_trap_next (long *pstart, long *pend)
{
  ...
  *pend = end; 
  ...
  while (1) {
	...
	if (__builtin_expect (tmp == start, 1)) {
//printf("%ld,",q);
//printf("%i, q: %ld, n: %ld, threadNr: %i, start: %ld, end: %ld, nend: %ld, trapstart: %i, decrdelta: %f\n", mynumber, q, n, omp_get_thread_num(), start, end,  nend, ws->startsize, ws->decr_delta);
	break;}
	start = tmp;
  }
    *pstart = start;
...
//struct timespec endtime;
//clock_gettime(CLOCK_MONOTONIC, &endtime);
//printf("diff: %f", diff(starttime, endtime));
	//printf("%ld fsc , thrNr: %i, end: %ld,  nend: %ld, chunk: %ld, fscsize: %i\n", start, omp_get_thread_num(), end,  nend, chunk, ws->fscsize );
	break;
	}
